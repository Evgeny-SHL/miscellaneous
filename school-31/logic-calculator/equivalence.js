...
submit.onsubmit = function() {								// Действия при клике на кнопку.
	...
	for (i = 0; i < Math.pow(2, n); i++) {										// Берем числа от нуля до 2^n-1
		...
		if (f1 != f2) {
			var str = 'Формулы не являются равносильными: при наборе переменных ';
			for (var j = 0; j < n-1; j++) {
				str += vars[j] + '≡' + code[j] + ', ';
			}
			str += vars[n-1] + '≡' + code[n-1] + ' эквиваленция формул ложна.'
	...
	result.appendChild(message).innerHTML = 'Формулы являются равносильными: ' + formula_1 + '≡' + formula_2 + '.';
	return false;
}