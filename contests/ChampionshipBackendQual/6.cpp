...
В целях экономии ресурсов тестирующая система может проверять только два коммита одновременно. Вам требуется написать программу, которая будет определять номер
m
.
...
class State {
 public:
  bool Add(size_t commit, size_t status) {
    m[commit] = status + 1;
    return Check(commit);
  }

 private:
  bool Check(size_t commit) {
    if (m[commit] == 2 && m[commit + 1] == 1) {
      std::cout << "! " << commit + 1 << std::endl;
      return true;
    }
    if (m[commit - 1] == 2 && m[commit] == 1) {
      std::cout << "! " << commit << std::endl;
      return true;
    }
    if (commit == 1 && m[commit] == 1) {
      std::cout << "! 1" << std::endl;
      return true;
    }
    return false;
...
std::tuple<size_t, size_t, size_t> Ask1(size_t x, size_t y, size_t z) {
  size_t a = 0;
  size_t b = 0;
  size_t c = 0;
  std::cin >> a >> b >> c;
...
std::tuple<size_t, size_t, size_t> Ask2(size_t x, size_t y, size_t z) {
  size_t m = 1;
  return {x < m ? 1 : 0, y < m ? 1 : 0, z < m ? 1 : 0};
}

std::tuple<size_t, size_t, size_t> Ask(size_t x, size_t y, size_t z) {
  return Ask1(x, y, z);
}
